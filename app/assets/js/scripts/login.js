        ...
        if(err.error != null){
            if(err.error === 'ForbiddenOperationException'){
                if(err.errorMessage != null){
                    if(err.errorMessage === "Identifiants invalides. Nom d'utilisateur ou mot de passe invalide."){
                        return {
...
loginButton.addEventListener('click', () => {
    ...
    if(loginPassword.value === '' && !validUsername.test(loginUsername.value)){
        setOverlayContent('Utilisateur Cracké', "Vous devez mettre un nom d'utilisateur valide !", 'Pas de problème !')
        setOverlayHandler(() => {
            formDisabled(false)
            toggleOverlay(false)
        })
