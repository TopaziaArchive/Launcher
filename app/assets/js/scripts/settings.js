...
function bindAuthAccountSelect(){
    Array.from(document.getElementsByClassName('settingsAuthAccountSelect')).map((val) => {
        val.onclick = (e) => {
            ...
            val.innerHTML = 'Compte Séléctionné &#10004;'
...
function refreshAuthAccountSelected(uuid){
    Array.from(document.getElementsByClassName('settingsAuthAccount')).map((val) => {
        ...
        if(uuid === val.getAttribute('uuid')){
            ...
            selBtn.innerHTML = 'Compte Séléctionné &#10004;'
        } else {
            if(selBtn.hasAttribute('selected')){
                selBtn.removeAttribute('selected')
            }
            selBtn.innerHTML = 'Séléctionner un Compte'
        }
    })
}
    ...
    authKeys.map((val) => {
        ...
        authAccountStr += `<div class="settingsAuthAccount" uuid="${acc.uuid}">
            <div class="settingsAuthAccountLeft">
                <img class="settingsAuthAccountImage" alt="${acc.displayName}" src="https://minotar.net/helm/${acc.displayName}">
            </div>
            <div class="settingsAuthAccountRight">
                ...
                <div class="settingsAuthAccountActions">
                    <button class="settingsAuthAccountSelect" ${selectedUUID === acc.uuid ? 'selected>Compte Séléctionné &#10004;' : '>Séléctionner un Compte'}</button>
...
function populateJavaExecDetails(execPath){
    ...
    jg._validateJavaBinary(execPath).then(v => {
        if(v.valid){
            ...
            settingsJavaExecDetails.innerHTML = 'Fichier Invalide'
        }
    })
}
...
function populateSettingsUpdateInformation(data){
    if(data != null){
        ...
        settingsUpdateTitle.innerHTML = 'Vous utilisez actuellement la dernière Version'
        settingsUpdateChangelogCont.style.display = 'none'
        populateVersionInformation(remote.app.getVersion(), settingsUpdateVersionValue, settingsUpdateVersionTitle, settingsUpdateVersionCheck)
        settingsUpdateButtonStatus('Vérifier des Mises à jour', false, () => {
            if(!isDev){
